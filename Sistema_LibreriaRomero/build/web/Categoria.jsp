    ...
    <form class="form-inline my-2 my-lg-0" action="Controlador" method="post">
        ...
        <div class="col-lg-6 mx-auto p-0">
            ...
            <br>
                 ...
                 <%
                        ResultSet rs = (ResultSet) request.getSession().getAttribute("ListaCate");
                        if (rs != null) {
                    %>
                ...
                <tbody>
                    <%
                        ...
                        %>
                    <tr>
                        <th><%= rs.getInt("IdCategoria")%></th>
                        <th><%= rs.getString("NombreCategoria")%></th>
                        ...
                        <th><%
                            ...
                            %></th>
                        <td><a class="btn btn-outline-primary" href="Controlador?accion=EditarCate&ID=<%=rs.getInt("IdCategoria")%>&Nom=<%= rs.getString("NombreCategoria")%>&Des=<%= rs.getString("Descripcion")%>&Est=<%= rs.getString("Estado")%>" role="button"><i class="bi bi-pencil"></i></a></td>
                        <td><a id="deleteCate" class="btn btn-outline-danger" onclick="Confirmar(<%=rs.getInt("IdCategoria")%>);"><i class="bi bi-eraser"></i></a></td>
                    </tr>
                    <% } %>
                </tbody>
                <% } %>
                ...
                function Confirmar(codigo) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            window.location.href = "Controlador?accion=EliminarCate&ID=" + codigo + "";
                        } else if (result.isDenied) {
                            Swal.fire('Changes are not saved', '', 'info');
                            window.location.href = "Controlador?accion=ListarCategoria";
                        }
                    });
                }
