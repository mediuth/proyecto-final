...
<html>
    ...
    <form class="form-inline my-2 my-lg-0" action="Controlador" method="post">
            ...
            <div class="row justify-content-center">
                <div class="col-sm-8"> 
                    <form>
                     <%
                        ResultSet rs = (ResultSet) request.getSession().getAttribute("ListaUsuarios");
                        if (rs != null) {
                    %>
                    <table class="table">
                        ...
                        %>
                        <tr>
                            <th><%= rs.getInt("IdUsuario")%></th>
                            <td><%= rs.getString("NombreCompleto")%></td>
                            <td><%= rs.getString("UserName")%></td>
                            <td><%= rs.getString("Pass")%></td>
                           <td><%= rs.getInt("IdRol")%></td>
                           <td><%= rs.getString("Email")%></td>
                           <td><%= rs.getString("Fecha")%></td>
                           <td><%= rs.getString("Estado")%></td>
                           
                            <td>
                                <a class="btn btn-warning" href="Controlador?accion=Editar&IdUsuario=<%=rs.getInt("IdUsuario")%>&NombreCompleto=<%= rs.getString("NombreCompleto")%>&UserName=<%= rs.getString("UserName")%>&Pass=<%= rs.getString("Pass")%>&IdRol=<%= rs.getInt("IdRol")%>&Email=<%= rs.getString("Email")%>&Fecha=<%= rs.getString("Fecha")%>&Estado=<%= rs.getString("Estado")%>"> Editar </a>
                               <a id="deleteUser" class="btn btn-danger" onclick="Confirmar(<%=rs.getInt("IdUsuario")%>);" >Eliminar </a>
                            </td>
                        </tr>
                        <%
                            }
  ...
  if (result.isConfirmed) {
    ...
    window.location.href ="Controlador?accion=Eliminar&IdUsuario="+IdUsuario+"";
