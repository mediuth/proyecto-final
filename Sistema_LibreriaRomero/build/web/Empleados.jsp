  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    ...
    <form class="form-inline my-2 my-lg-0" action="Controlador" method="post">
                ...
                <div class="col-sm-8"> 
                    ...
                    <%
                        ResultSet rs = (ResultSet) request.getSession().getAttribute("ListarEmpleados");
                        if (rs != null) {
                    %>
                     <table class="table table-striped table-hover">
                    <thead>
                        <tr>
                            <th scope="row" colspan="4">Listado de Empleados</th>
                        </tr>
                        <tr>
                            <td>ID</td>
                            <td>Nombre </td>
                        ...
                        %>
                        <tr>
                            <th name="id"><%= rs.getInt("IdEmpleado")%></th>
                            ...
                            <td>
                                <a class="btn btn-outline-primary" href="Controlador?accion=Editar_Emp&IdEmpleado=<%=rs.getInt("IdEmpleado")%>&Nombre=<%= rs.getString("Nombre")%>&Email=<%= rs.getString("Email")%>&Telefono=<%= rs.getString("Telefono")%>&FechaNac=<%= rs.getString("FechaNac")%>&Direccion=<%= rs.getString("Direccion")%>&Dui=<%= rs.getString("Dui")%>&Nit=<%= rs.getString("Nit")%>&Area=<%= rs.getString("Area")%>" role="button"><i class="bi bi-pencil"></i> </a>
                                
                                <td><a id="deleteUser" class="btn btn-outline-danger" onclick="Confirmar(<%=rs.getInt("IdEmpleado")%>);"><i class="bi bi-eraser"></i></a></td>
                            </td>
                ...
                function Confirmar(IdEmpleado) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            window.location.href ="Controlador?accion=Eliminar_Emp&IdEmpleado="+IdEmpleado+"";
                        } else if (result.isDenied) {
                            Swal.fire('Changes are not saved', '', 'info');
                            window.location.href ="Controlador?accion=Principal";
                        }
                    });
                }
